$(function() {
  ...
  var ref = firebase.database().ref().child("to-fix");
	...
	$('#action_item').click(function() {
		...
		$('#playlist').addClass('col s8 push-s4 to-fix-list');
		ref.on('child_added', function(snapshot) {
			if(snapshot.val().type === "message") {
	  	$('.to-fix-list').append('<div class=interior id='+snapshot.key+'> <span class="title">Message </span>'+snapshot.val().message+'<p><b>Date:'+snapshot.val().timestamp+'</b><br><b>Timestamp:</b></p><button class=button-' + snapshot.key + '>YES</button></div><div class="divider"></div>');
